      ...
      <div class="area">
        ...
        <div class="button-list">
          <div class="button-wrapper"
               ...
               :key="item.id">
            <div class="button"
                 @click="cityToStore(item.name)">{{item.name}}</div>
          </div>
        </div>
      </div>
      <!-- 
      ...
      <div class="area"
        ...
        <div class="item-list"
          ...
          <div class="item border-bottom"
               @click="cityToStore(inneritem.name)"> {{inneritem.name}}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  watch: {
    accerindex() {
      if (this.accerindex) {
        ...
        // console.log(this.$refs[this.accerindex])
        //返回数组，这里accerindex是字符串，也是循环的key也正是,A,B
        let citydom = this.$refs[this.accerindex][0]
        // console.log(citydom)
